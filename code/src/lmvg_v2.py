...
class LMVGv2(nn.Module):
    def __init__(self, n_ch=10, T=14, tau_init=-1.0):
        super().__init__()
        self.n_ch = n_ch
        self.T = T
        self.tau = nn.Parameter(torch.full((n_ch,), float(tau_init)))
        self.view_logits = nn.Parameter(torch.tensor([0.5, 0.3, 0.2]))
        self.hard = False
        self._last_adj = None
    ...
    def _gumbel_edge(self, logits, tau_ch, temperature):
        tau_b = tau_ch.expand_as(logits)
        stacked = torch.stack([logits, tau_b], dim=-1)
    ...
    def forward(self, x_btc, temperature=1.0):
        ...
        stats = dict(
            ...
            hard_mode=bool(self.hard),
        )
        return fused, stats

    def structure_loss(self, target_sparsity=0.25):
        if self._last_fused is None:
            return torch.tensor(0.0, device=self.tau.device)
        sp = self._last_fused.mean()
        sp_loss = (sp - target_sparsity).pow(2)
        w = F.softmax(self.view_logits, dim=0)
        entropy = -(w * (w + 1e-8).log()).sum()
        return sp_loss - 0.1 * entropy
