...
from graph_backend_tuned import make_model_tuned
from lmvg_tuned import hamming_distance, cosine_temperature
...
def clone_wrapper(model, backend):
    clone = make_model_tuned(backend=backend)
...
def meta_train_step(meta_model, backend, src_batches, tgt_batch, inner_lr, inner_steps, mmd_w):
    ...
    avg_grads = [(g / n) if g is not None else None for g in meta_grads]
    return avg_grads, total_q / n, total_m / n, None


def apply_meta_grads_grouped(meta_model, meta_grads, lr_base, lr_lmvg):
    lmvg_set = meta_model.lmvg_param_names()
    names = [n for n, _ in meta_model.named_parameters()]
    with torch.no_grad():
...
def run_one(backend, X, y, tasks, players, dates_all, tgt_task, seed, log_rows,
            episodes=200, patience=20, meta_lr_base=2e-3, meta_lr_lmvg=5e-3):
    ...
    meta_model = make_model_tuned(backend=backend)
    ...
    for ep in range(1, episodes + 1):
        ...
        grads, q_loss, mmd_v, _ = meta_train_step(meta_model, backend, src_batches, tgt_batch, inner_lr, inner_steps, mmd_w)
        if grads is None:
            continue
        ...
        print(
            ...
            f"tau={stats['tau_mean']:+.2f}±{stats['tau_std']:.2f} ||∇τ||={tau_gn:.1e} "
            f"w=[{vw[0]:.2f},{vw[1]:.2f},{vw[2]:.2f}] H={stats['view_entropy']:.2f} "
            f"Tg={t_g:.2f} sp={stats['sparsity']:.3f} ham={ham:.3f} "
            f"Δτ={stats['tau_shift']:.3f} Δvl={stats['vl_shift']:.3f} "
        ...
        if flag_lmvg == "LMVG_INEFFECTIVE":
            print(f"  [GUARD-HALT-LMVG] ΔAUC<-0.01 for 5 eps at ep{ep}, rollback-early-stop")
            break
...
def main():
    ...
    t0 = time.time()
    configs = [
        ("lmvg", dict(episodes=200, patience=20, meta_lr_base=2e-3, meta_lr_lmvg=5e-3)),
    ...
    for tgt in ABL_TARGETS:
        for backend in ["lmvg", "nvg"]:
            ...
            summary[(tgt, backend)] = (np.mean(aucs), np.std(aucs))
