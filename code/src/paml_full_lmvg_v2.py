...
def clone_wrapper(model, backend):
    clone = make_model_v2(backend=backend)
    clone.load_state_dict(copy.deepcopy(model.state_dict()))
    clone.set_gumbel_temp(model._gumbel_temp)
    if backend == "lmvg":
        clone.set_hard(model.lmvg.hard)
    return clone
...
def inner_update(base_model, backend, x_s, y_s, inner_lr, inner_steps):
    ...
    for _ in range(inner_steps):
        ...
        if backend == "lmvg" and getattr(fast, "lmvg", None) is not None:
...
def meta_train_step(meta_model, backend, src_batches, tgt_batch, inner_lr, inner_steps, mmd_w):
    ...
    for task in src_batches:
        ...
        if backend == "lmvg" and getattr(fast, "lmvg", None) is not None:
            loss = loss + 2.0 * fast.lmvg.structure_loss()
        trainable_named = [(name, p) for name, p in fast.named_parameters() if p.requires_grad]
        trainable_params = [p for _, p in trainable_named]
        grads_only = torch.autograd.grad(loss, trainable_params, allow_unused=True)
        grad_dict = {name: g for (name, _), g in zip(trainable_named, grads_only)}
        grads = [grad_dict.get(name, None) for name, _ in fast.named_parameters()]
...
def apply_meta_grads_grouped(meta_model, meta_grads, lr_base, lr_lmvg):
    lmvg_set = meta_model.lmvg_param_names()
    with torch.no_grad():
        for (name, p), g in zip(meta_model.named_parameters(), meta_grads):
            if g is None:
                continue
            lr = lr_lmvg if name in lmvg_set else lr_base
            p.data -= lr * g


def compute_tau_grad_norm(meta_model, meta_grads):
    if meta_model.backend != "lmvg":
        return 0.0
    for (n, p), g in zip(meta_model.named_parameters(), meta_grads):
        if n == "lmvg.tau" and g is not None:
...
def eval_on_target(model, X_te, y_te):
    ...
    if y_te.sum() < 1 or len(np.unique(y_te)) < 2:
        return None
...
def fine_tune(model, X_sup, y_sup, steps=50, lr=1e-3):
    ...
    for _ in range(steps):
        ...
        if getattr(model, "lmvg", None) is not None:
...
def run_one(backend, X, y, tasks, players, dates_all, tgt_task, seed, log_rows,
            episodes=200, patience=20, meta_lr_base=2e-3, meta_lr_lmvg=5e-1,
            n_sup_pos_override=None):
    ...
    rng.shuffle(pos_idx)
    rng.shuffle(neg_idx)
    if n_sup_pos_override is not None:
        n_sup_pos = min(int(n_sup_pos_override), max(len(pos_idx) - 2, 1))
    else:
    ...
    for ep in range(1, episodes + 1):
        if backend == "lmvg":
            ...
            if not hard_switched and ep >= HARD_SWITCH_EP:
                ...
                hard_switched = True
                print(f"  [SWITCH-HARD] ep{ep} switched to hard Gumbel")
        else:
            t_g = 0.0
        ...
        if grads is None:
            continue
        tau_gn = compute_tau_grad_norm(meta_model, grads)
        ...
        if backend == "lmvg":
            if tau_gn < 1e-5:
                ...
                low_grad_streak = 0
            if low_grad_streak >= 20:
                flag_lmvg = "LMVG_STUCK"
            delta_auc = r_te["auc"] - nvg_ref
            if delta_auc < -0.01:
                ineffective_streak += 1
            else:
                ineffective_streak = 0
            if ineffective_streak >= 80:
                flag_lmvg = "LMVG_INEFFECTIVE"
        ...
        row = dict(
            ...
            gumbel_temp=round(t_g, 4), view_entropy=round(stats["view_entropy"], 4),
            tau_grad_norm=round(tau_gn, 6),
            tau_shift=round(stats["tau_shift"], 4), vl_shift=round(stats["vl_shift"], 4),
            hard_mode=bool(stats.get("hard_mode", False)),
            delta_vs_nvg=round(r_te["auc"] - nvg_ref, 4),
            flag_overfit=flag_overfit, flag_qloss=flag_qloss, flag_mmd=flag_mmd,
            flag_drift=flag_drift, flag_lmvg=flag_lmvg,
        )
        epoch_log.append(row)
        log_rows.append(row)
        if ep % 5 == 1 or ep <= 10:
            print(
                f"  ep{ep:03d}[{backend}] qL={q_loss:.4f} mmd={mmd_v:.3f} "
                f"teAUC={r_te['auc']:.3f}(Δ{r_te['auc']-nvg_ref:+.3f}) "
                f"tau={stats['tau_mean']:+.3f}±{stats['tau_std']:.3f} ||∇τ||={tau_gn:.1e} "
                f"w=[{vw[0]:.2f},{vw[1]:.2f},{vw[2]:.2f}] H={stats['view_entropy']:.2f} "
                f"Tg={t_g:.2f} hard={stats.get('hard_mode', False)} sp={stats['sparsity']:.3f} "
                f"ham={ham:.3f} Δτ={stats['tau_shift']:.3f} Δvl={stats['vl_shift']:.3f} "
                f"[{flag_overfit}|{flag_qloss}|{flag_mmd}|{flag_drift}|{flag_lmvg}]"
            )

        if flag_overfit == "OVERFIT" and ep >= 20:
        ...
        if flag_drift == "DRIFT_HALT":
            ...
            break
        if flag_lmvg == "LMVG_INEFFECTIVE":
            print(f"  [GUARD-HALT-LMVG] ΔAUC<-0.01 for 15 eps at ep{ep}")
            break
...
def main():
    ...
    t0 = time.time()
    configs = [
        ("lmvg", dict(episodes=200, patience=20, meta_lr_base=2e-3, meta_lr_lmvg=5e-1)),
        ("nvg",  dict(episodes=50,  patience=8,  meta_lr_base=1e-3, meta_lr_lmvg=1e-3)),
    ]
    quick_mode = False
    if quick_mode:
        quick_targets = ["TeamA-2020"]
        quick_seeds = [42]
    else:
        quick_targets = ABL_TARGETS
        quick_seeds = SEEDS
    for backend, cfg in configs:
        for tgt in quick_targets:
            for seed in quick_seeds:
                print(f"\n===== backend={backend} target={tgt} seed={seed} [{(time.time()-t0)/60:.1f}min] =====")
                res = run_one(backend, X, y, tasks, players, dates, tgt, seed, log_rows, **cfg)
    ...
    with open(os.path.join(here, "lmvg_tuned_results.json"), "w") as f:
        json.dump([clean(r) for r in all_results], f, indent=2, default=str)
    pd.DataFrame(log_rows).to_csv(os.path.join(here, "lmvg_tuned_epoch_log.csv"), index=False)

    print("\n============ TUNED (B1+B3) AGGREGATE ============")
    summary = {}
    for tgt in ABL_TARGETS:
        for backend in ["lmvg", "nvg"]:
            ...
            summary[(tgt, backend)] = (float(np.mean(aucs)), float(np.std(aucs)))
    ...
    print("\n============ COMPARE: tuned-LMVG vs NVG vs ORIG-LMVG ============")
    orig_lmvg = {"TeamA-2020": 0.9012, "TeamA-2021": 0.8912, "TeamB-2020": 0.9862}
    deltas = []
    for tgt in ABL_TARGETS:
        if (tgt, "lmvg") not in summary or (tgt, "nvg") not in summary:
            continue
        a_tuned = summary[(tgt, "lmvg")][0]
        a_nvg = summary[(tgt, "nvg")][0]
        a_orig = orig_lmvg.get(tgt, float("nan"))
        d1 = a_tuned - a_nvg
        d2 = a_tuned - a_orig
        deltas.append(d1)
        print(f"  {tgt}: tuned-LMVG={a_tuned:.4f}  NVG={a_nvg:.4f}  orig-LMVG={a_orig:.4f}  "
              f"Δ(tuned-NVG)={d1:+.4f}  Δ(tuned-orig)={d2:+.4f}")
    if deltas:
        print(f"  MEAN Δ(tuned-LMVG − NVG) = {np.mean(deltas)*100:+.2f} pp (target ≥ +0.30 pp)")
